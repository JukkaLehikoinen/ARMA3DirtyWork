...
//ostohinta = asehinta select _indexi;
//ostotuote = tuote select _indexi;
...
if (_tuote == tuote2 select _x) then {
	ostotuote = tuote2 select _x;
	ostohinta = asehinta2 select _x;
	};
};

for "_x" from 0 to lipaslaskuri do {

if (_tuote == lipas2 select _x) then {
	ostotuote = lipas2 select _x;
	ostohinta = lipashinta2 select _x;
	};
};
