...
if (_tuote == tuote4 select _x) then {
	ostotuote = tuote4 select _x;
	ostohinta = asehinta4 select _x;
	};
};

for "_x" from 0 to lipaslaskuri do {

if (_tuote == lipas4 select _x) then {
	ostotuote = lipas4 select _x;
	ostohinta = lipashinta4 select _x;
	};
};
